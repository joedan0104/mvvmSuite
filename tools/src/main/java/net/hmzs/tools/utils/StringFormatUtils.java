...
public class StringFormatUtils {
    ...
    public static String twoDecimalFormat(Object args) {
        ...
        return df.format(StringUtils.getDouble(args.toString()));
    }
    ...
    public static String bankcardFormat(String args) {
        if (android.text.TextUtils.isEmpty(args)) {
            return args;
        }
    ...
    public static String IDCardFormat(String IDCard) {
        if (android.text.TextUtils.isEmpty(IDCard)) {
            return IDCard;
        }
    ...
    public static String doubleFormat(Object args) {
        if (args != null && !StringUtils.isEmpty(args.toString())) {
            String number = args.toString();
            try {
                DecimalFormat df = new DecimalFormat();
                if (StringUtils.getDouble(number) < 1) {
                    df.applyPattern("0.00");
                } else {
                    df.applyPattern("##,###,###,###.00");
                }
                return df.format(StringUtils.getDouble(number));
            } catch (Exception e) {
                e.printStackTrace();
                return number;
            }
    ...
    public static String doubleMoney(Object formatArgs, int type) {
        if (formatArgs != null && !android.text.TextUtils.isEmpty(formatArgs.toString())) {
    ...
    public static String doubleFormatForW(Object args) {
        if (null != args && !TextUtils.isEmpty(args.toString())) {
            String number = args.toString();
            try {
                double digit = StringUtils.getDouble(number);
                if (digit >= 10000) {
