...
public class FileSizeUtils {
    ...
    private static double FormatFileSize(long size, int sizeType) {
        ...
        switch (sizeType) {
            case SIZE_TYPE_B:
                fileSizeLong = StringUtils.getDouble(df.format((double) size));
                break;
            case SIZE_TYPE_KB:
                fileSizeLong = StringUtils.getDouble(df.format((double) size / 1024));
                break;
            case SIZE_TYPE_MB:
                fileSizeLong = StringUtils.getDouble(df.format((double) size / 1048576));
                break;
            case SIZE_TYPE_GB:
                fileSizeLong = StringUtils.getDouble(df.format((double) size / 1073741824));
                break;
            default:
                break;
        }
